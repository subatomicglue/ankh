...
class NoneMethod : public BehaviourMethod { public:   virtual void doThing( BehaviourMethod::Thing, AnkhActor* parent, AnkhActor* caller ) {} };
typedef NoneMethod TeleMethod;
typedef BadguyMethod BenignMethod;
...
class OscMethod : public BehaviourMethod
{
public:
   OscMethod( s32 dirx, s32 diry ) : m_oDir( gba::Vec<s32>( dirx, diry ) )
   {
   }
      ...
      switch (thing)
      {
         case BehaviourMethod::UPDATE:
         {
            ...
            if (intersectAllActors( *parent, ACTOR_COLLISION_PHYSICS ) || intersectMap( *parent ))
            {
               // undo, can't go there...
               parent->m_oPos = old_pos;
               m_oDir = -m_oDir; // invert direction.
            }
         }
         break;
         case BehaviourMethod::TOUCH:
